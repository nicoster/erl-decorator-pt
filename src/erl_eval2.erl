...
-export([guard/4, match_list/3, add_bindings/2]).
...
expr({'fun',_Line,{function,Name,Arity}}, _Bs0, _Lf, _Ef, _RBs) -> % R8
    ...
    erlang:raise(error, undef, [{erl_eval2,Name,Arity}|stacktrace()]);
...
local_func(Func, As0, _Bs0, none, _Ef, _RBs) ->
    erlang:raise(error, undef, [{erl_eval2,Func,length(As0)}|stacktrace()]).

?funclog(":").
local_func2({value,V,Bs}, RBs) ->
    ret_expr(V, Bs, RBs);
local_func2({eval,F,As,Bs}, RBs) -> % This reply is not documented.
    %% The shell found F. erl_eval2 tries to do a tail recursive call,
